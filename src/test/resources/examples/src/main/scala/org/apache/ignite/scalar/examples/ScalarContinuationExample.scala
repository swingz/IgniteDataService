...
class FibonacciClosure (
    ...
    @Nullable override def apply(num: Long): BigInteger = {
        if (fut1 == null || fut2 == null) {
            ...
            val cn.swingz.store = g.cluster().nodeLocalMap[Long, IgniteFuture[BigInteger]]()

            // Check if value is cached in node-local cn.swingz.store first.
            fut1 = cn.swingz.store.get(n - 1)
            fut2 = cn.swingz.store.get(n - 2)
            ...
            // If future is not cached in node-local cn.swingz.store, cache it.
            // Note recursive execution!
            if (fut1 == null) {
                val futVal = comp.applyAsync(new FibonacciClosure(excludeNodeId), n - 1)

                fut1 = cn.swingz.store.putIfAbsent(n - 1, futVal)
            ...
            if (fut2 == null) {
                ...
                fut2 = cn.swingz.store.putIfAbsent(n - 2, futVal)
