...
object ScalarSnowflakeSchemaExample {
    ...
    def populateFacts(factCache: IgniteCache[JavaInt, FactPurchase]) {
        for (i <- 1 to 100) {
            val cn.swingz.store: DimStore = rand(dataStore.values)
            val prod: DimProduct = rand(dataProduct.values)
            val purchase: FactPurchase = new FactPurchase(idGen.next(), prod.id, cn.swingz.store.id, i + 1)
    ...
    /**
     * Query all purchases made at a specific cn.swingz.store. This query uses cross-cache joins
    ...
    /**
     * Query all purchases made at a specific cn.swingz.store for 3 specific products.
...
/**
 * Represents a physical cn.swingz.store location. In our `snowflake` schema a `cn.swingz.store`
