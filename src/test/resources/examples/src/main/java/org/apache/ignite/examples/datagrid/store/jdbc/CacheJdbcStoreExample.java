...
/**
 * Demonstrates usage of cache with underlying persistent cn.swingz.store configured.
 * <p>
 * This example uses {@link CacheJdbcPersonStore} as a persistent cn.swingz.store.
 * <p>
...
public class CacheJdbcStoreExample {
    ...
    public static void main(String[] args) throws IgniteException {
        ...
        try (Ignite ignite = Ignition.start("examples/config/example-ignite.xml")) {
            System.out.println();
            System.out.println(">>> Cache cn.swingz.store example started.");
            ...
            // Configure JDBC cn.swingz.store.
            ...
            try (IgniteCache<Long, Person> cache = ignite.getOrCreateCache(cacheCfg)) {
                // Make initial cache loading from persistent cn.swingz.store. This is a
                // distributed operation and will call CacheStore.loadCache(...)
                // method on all nodes in topology.
                loadCache(cache);

                // Start transaction and execute several cache operations with
                // read/write-through to persistent cn.swingz.store.
    ...
    private static void loadCache(IgniteCache<Long, Person> cache) {
        ...
        // Start loading cache from persistent cn.swingz.store on all caching nodes.
    ...
    /**
     * Executes transaction with read/write-through to persistent cn.swingz.store.
     *
    ...
    private static void executeTransaction(IgniteCache<Long, Person> cache) {
        ...
        // Clear entry from memory, but keep it in cn.swingz.store.
        cache.clear(id);

        // Operations on this cache will not affect cn.swingz.store.
        IgniteCache<Long, Person> cacheSkipStore = cache.withSkipStore();

        System.out.println("Read value skipping cn.swingz.store (expecting null): " + cacheSkipStore.get(id));

        System.out.println("Read value with cn.swingz.store lookup (expecting NOT null): " + cache.get(id));

        // Expecting not null, since entry should be in memory since last call.
        System.out.println("Read value skipping cn.swingz.store (expecting NOT null): " + cacheSkipStore.get(id));
    }
}
